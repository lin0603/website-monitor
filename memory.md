...
## 專案概述
網站運行狀態監控器，具備 Web Dashboard 和 Telegram Bot 功能。
...
curl -s -X POST https://captain.gcp-caprover.starxinteractive.com/api/v2/user/apps/appDefinitions/register \
  ...
  -d '{"appName":"website-monitor","hasPersistentData":false}'
```

### 步驟 3：設定環境變數和 Instance Count
```bash
curl -s -X POST https://captain.gcp-caprover.starxinteractive.com/api/v2/user/apps/appDefinitions/update \
  ...
  -d '{
    ...
    "instanceCount": 1,
    "envVars": [
      {"key": "MONITOR_URLS", "value": "https://google.com,https://translinebot.starxinteractive.com/vietnam"},
...
| Key | Value | 說明 |
|-----|-------|------|
| `MONITOR_URLS` | `https://google.com,https://translinebot.starxinteractive.com/vietnam` | 監測目標，逗號分隔 |
...
| `CHECK_INTERVAL` | `300` | 檢查間隔（秒），5 分鐘 |
